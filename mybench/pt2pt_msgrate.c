...
void thread_func(void *arg)
{
    ...
    if (comm_type == COMM_TYPE_PT2PT) {
        requests =
            (MPI_Request *)malloc(sizeof(MPI_Request) * window_size + 128);
        ...
        for (int i = 0; i < g_num_threads; i++) {
            ...
            if (i != tid)
                continue;
            ret = MPI_Win_allocate(buf_size, 1, MPI_INFO_NULL, comm,
                                   (void *)&buf, &win);
            assert(ret == 0);
    ...
    for (int i = 0; i < 1 + g_num_repeats; i++) {
        ...
        if (g_debug_mode > 0) {
            ...
            MPIX_Set_exp_info(MPIX_INFO_TYPE_DEBUG_ENABLED, NULL,
                              g_debug_mode ? 1 : 0);
            MPIX_Set_exp_info(MPIX_INFO_TYPE_PRINT_ENABLED, NULL,
                              g_debug_mode - 1);
        }
        if (g_nolock_mode) {
        ...
        for (size_t win_post_i = 0; win_post_i < win_posts; win_post_i++) {
            int is_sender =
                (g_comm_rank & 1) ? (win_post_i % 2) : (1 - win_post_i % 2);
            if (comm_type == COMM_TYPE_PT2PT) {
                for (size_t win_i = 0; win_i < window_size; win_i++) {
                    if (is_sender) {
                        ret = MPI_Isend(&buf[win_i * message_size],
                                        message_size, MPI_CHAR, target, tid,
                                        comm, &requests[win_i]);
                        assert(ret == 0);
                    } else {
                        ret = MPI_Irecv(&buf[win_i * message_size],
                                        message_size, MPI_CHAR, target, tid,
                                        comm, &requests[win_i]);
                        assert(ret == 0);
                    }
...
void main_thread_func(void *arg)
{
    ...
    if (g_benchmark_type == 0) {
        /* MPI everywhere */
        int provided;
        MPI_Init_thread(p_arg->p_argc, p_arg->p_argv, MPI_THREAD_SINGLE,
                        &provided);
        ...
        /* Multithreaded. */
        int provided;
        MPI_Init_thread(p_arg->p_argc, p_arg->p_argv, MPI_THREAD_MULTIPLE,
                        &provided);
    ...
    if (g_comm_rank == 0) {
        ...
        const size_t win_posts = g_num_messages / g_window_size + 1;
        double total_num_messages =
            ((double)win_posts) * g_window_size * g_num_entities;
...
int main(int argc, char *argv[])
{
    if (argc != 7) {
        ...
        printf("BENCHMARK_TYPE = 0 : MPI everywhere / 2 * NUM_ENTITIES "
               "processes\n");
