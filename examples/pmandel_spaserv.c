...
void check_mand_params(int *m_max_iterations,
                       ...
                       double *m_divergent_limit, int *m_num_children);
    ...
    if (parent) {
        ...
        for (;;) {
            ...
            while (k < 400) {
                ...
                MPI_Irecv(&result[index], 5, MPI_INT, 0, 200, child_comm[index],
                          &child_request[index]);
                k++;
            }
...
void output_data(int *in_grid_array, int coord[4], int *out_grid_array, int width, int height)
{
    ...
    if (!use_stdin) {
        ...
        error =
            MPI_Send(in_grid_array, (coord[1] + 1 - coord[0]) * (coord[3] + 1 - coord[2]), MPI_INT,
                     0, 0, comm);
        if (error != MPI_SUCCESS) {
